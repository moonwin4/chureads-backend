 ...
 */
export const generateTags = async (content, OPENAI_API_KEY) => {
    ...
    try {
        ...
        //console.log("🚀 ~ message:", response.choices.message);
        ...
        //const data = [...messages, response.choices[0].message.content];
...
export const testTagGenerate = async (OPENAI_API_KEY) => {
    testContents.forEach(async (content, index) => {
        console.log(`================= 테스트 ${index} =====================`);
        //console.log("🚀 원본:", content);
        const result = await generateTags(content, OPENAI_API_KEY);
